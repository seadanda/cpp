...
class complx // class for complex numbers
{
...
public:
  ...
  complx(double re, double im) : real{re}, imaginary{im} {}
  // destructor
  ~complx() {}
  ...
  complx get_conjugate() const;

  // member functions to overload + and - operators
  complx operator+(const complx &z2) const;
  complx operator-(const complx &z2) const;
...
complx complx::get_conjugate() const // return complex conjugate
{
  complx temp{real, imaginary}; // clone input complex number
  temp.set_real(real);
  if (imaginary != 0) {
...
complx complx::operator+(const complx &z2) const // return sum
{
  complx tempz{real + z2.real, imaginary + z2.imaginary};
  return tempz;
}

// define the subtraction of two complex numbers
complx complx::operator-(const complx &z2) const // return difference
{
  complx temp{real - z2.real, imaginary - z2.imaginary};
  return temp;
}

// define the product of two complex numbers
complx complx::operator*(const complx &z2) const // return product
{
...
complx complx::operator/(const complx &z2) const // return quotient
{
...
ostream &operator<<(ostream &outs, const complx &z) // define ostream behaviour
{
  if (z.imaginary < 0) {
...
complx operator>>(istream &ins, complx &z) // define istream behaviour
{
  double re; // real part
